...
class QformerConnector(nn.Module):
    def __init__(self, config):
        ...
        self.layer_prompts = nn.ParameterList([
            nn.Parameter(torch.randn(1, self.config.prompt_size, self.config.encoder_config.d_model, dtype=torch.bfloat16))
            for _ in range(len(self.config.target_layer_ids))
        ])

        self.layer_weights = nn.Parameter(torch.zeros(self.config.prompt_size, len(self.config.target_layer_ids), dtype=torch.bfloat16))
        ...
        if self.config.connector_mode == "qformer_1":
            ...
            self.qformer = self.qformer.to(dtype=torch.bfloat16)
            ...
            self.proj = self.proj.to(dtype=torch.bfloat16)
        else:
...
class DeSTA25AudioModel(PreTrainedModel):
    ...
    def __init__(self, config, cache_dir=None, token=None, **kwargs):
        ...
        token = token if token else os.getenv("_TOHFKEN")
        ...
        self.perception = self.perception.to(dtype=torch.bfloat16)
    ...
    def _prepare_inputs_for_llm(self, 
        ...
        if not batch_start_positions:
            return self.llm_model.model.embed_tokens(input_ids)

        if batch_features is None or batch_transcription_ids is None:
            raise ValueError("Audio features and transcriptions must be provided when editing audio inputs.")
    ...
    def generate(self, messages,
        ...
        if len(all_audios) > 0:
            ...
            batch_features = batch_features.to(device=self.device, dtype=model_dtype)
            ...
            if asr_features:
                ...
                asr_features = asr_features.to(device=self.device, dtype=model_dtype)
