        ...
        for i in range(config.n_iter):
            ...
            if self.config.retrieve and self.get_adapter_layer().merge_cnt > 0 and self.config.replay:
                ...
                for _ in merge_group_edit_history:
                    ...
                    memo_input = _move_to_device(memo_input, self.config.device)
...
class WISEMultimodal(WISE):
    def edit(
        ...
        global edit_history
        global merge_group_edit_history
        edit_history.append([_detach_to_cpu(edit_batch), False])
        ...
        for i in range(config.n_iter):
            ...
            ft_loss, edit_delta = self._forward_edit_batch(edit_batch)

            locality_deltas = []
            if text_locality_inputs is not None:
                delta = self._forward_locality_batch(text_locality_inputs)
                if delta is not None:
                    locality_deltas.append(delta)
            if audio_locality_inputs is not None:
                delta = self._forward_locality_batch(audio_locality_inputs)
                if delta is not None:
                    locality_deltas.append(delta)

            act_loss = self._compute_activation_penalty(edit_delta, locality_deltas, ft_loss.device)
    ...
    def _forward_edit_batch(self, edit_batch):
        outputs = self.model(**edit_batch)
        ft_loss = self._compute_ft_loss_from_logits(edit_batch, outputs.logits)
        edit_delta = self._capture_current_delta()
        return ft_loss, edit_delta

    def _forward_locality_batch(self, locality_batch):
        if locality_batch is None:
            return None
        self.model(**locality_batch)
        return self._capture_current_delta()

    def _capture_current_delta(self):
        adapter_layer = super().get_adapter_layer()
        if not hasattr(adapter_layer, "original_layer_output") or adapter_layer.original_layer_output is None:
            return None
        return euc(adapter_layer.original_layer_output, adapter_layer.new_weight_layer_output, self.config)

    def _compute_activation_penalty(self, edit_delta, locality_deltas, device):
        total_loss = torch.tensor(0.0, device=device)
        edit_value = None
        if edit_delta is not None and edit_delta.numel() > 0:
            edit_value = edit_delta.mean()
            total_loss = total_loss + torch.relu(self.config.beta - edit_value)
        for delta in locality_deltas:
            if delta is None or delta.numel() == 0:
                continue
            loc_value = delta.mean()
            total_loss = total_loss + torch.relu(loc_value - self.config.alpha)
            if edit_value is not None:
                total_loss = total_loss + torch.relu(loc_value - edit_value + self.config.gamma)
        return total_loss

    def _compute_ft_loss_from_logits(self, batch, logits):
        labels = batch.get("labels")
        if labels is None:
            raise ValueError("Edit batch must include labels for fine-tuning loss.")
        ...
        label_mask = (flat_labels != -100).float()
        active = label_mask.sum()
        if active.item() == 0:
            return loss.new_tensor(0.0)
        return (loss * label_mask).sum() / active

    def _cal_ft_loss(self, edit_batch):
        logits = self.model(**edit_batch).logits
        shift_logits = logits[:, :-1, :].contiguous()
        labels = edit_batch.get("labels")
        if labels is None:
            raise ValueError("Multimodal edit batch must include supervision labels for fine-tuning loss.")
        shift_labels = labels[:, 1:].contiguous()
        ...
        loss = (loss * label_mask).sum() / active
        return loss
