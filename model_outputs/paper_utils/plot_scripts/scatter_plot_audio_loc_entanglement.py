...
ENTANGLEMENT_TYPE_DISPLAY = {
    "Type 1 Entanglement": "Inter-Loc.",
    "Type 2 Entanglement": "Intra-Loc.",
    "Type 4 Entanglement": "Cap-Loc.",
}
...
# --- Adjustable Plot Configurations ---
DOT_SIZE_ALL = 250
DOT_SIZE_DESTA = DOT_SIZE_ALL
DOT_SIZE_QWEN = DOT_SIZE_ALL
DOT_SIZE_AF = DOT_SIZE_ALL
DOT_SIZE_DEFAULT = DOT_SIZE_ALL
...
MODEL_STYLE = {
    "DeSTA2.5": {"size": DOT_SIZE_DESTA},
    "Qwen2-Audio": {"size": DOT_SIZE_QWEN},
    "Audio Flamingo 3": {"size": DOT_SIZE_AF},
}
MODEL_HATCH = {
    "DeSTA2.5": "//////",
    "Qwen2-Audio": "",
...
LEGEND_HANDLETEXTPAD = 0.2  # Adjusts the space between legend marker and text
LEGEND_FONTSIZE = 10
...
def load_audio_loc_metrics(single_csv_path):
    ...
    for row in rows[2:]:
        ...
        if method_cell:
            if method_cell == "FT (LLM)":
                current_method = "LMT"
            elif method_cell == "FT (Audio)":
                current_method = "PCT"
            else:
...
def print_raw_metrics_table(
    audio_loc_metrics_map, entanglement_map, entanglement_types
):
    ...
    print("Model,Method," + ",".join(audio_loc_headers) + "," + ",".join(type_headers))
...
def render_scatter(points, output_path, entanglement_types, annotate=False):
    if not points:
        raise ValueError(
            "No overlapping model-method pairs found between the two CSVs."
        )
    ...
    default_style = {"size": DOT_SIZE_DEFAULT}
    model_to_style = {
    ...
    for point in points:
        ...
        if annotate:
            ...
            short_type = ENTANGLEMENT_TYPE_DISPLAY.get(
                point["ent_type"], point["ent_type"]
            )
            axis.annotate(
    ...
    axis.set_xlabel("Audio Locality Score (%)", labelpad=2, fontsize=X_LABEL_FONTSIZE)
    axis.set_ylabel("Entanglement Rate (%)", labelpad=-3, fontsize=Y_LABEL_FONTSIZE)
    ...
    fig.legend(
        ...
        title="Model (Texture)",
        loc="lower left",
        bbox_to_anchor=(LEGEND_X_START + 0.314, LEGEND_COMMON_Y),
        frameon=True,
    ...
    fig.legend(
        ...
        title="Method (Marker)",
        loc="lower left",
        bbox_to_anchor=(LEGEND_X_START + 0.54, LEGEND_COMMON_Y),
        ncol=4,
