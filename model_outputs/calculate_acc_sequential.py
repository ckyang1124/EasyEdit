...
def update_global_acc(global_acc, acc):
    ...
    for gap in range(EDITS_PER_SEQ):
        ...
        for j in range(4):
            for key in keys:
                global_acc[gap]["loc"]["audio"][j][key] += acc[gap]["loc"]["audio"][j][key]
...
def main():
    ...
    parser.add_argument("--input_folder", "-i", type=str, required=True, help="Path to the input folder containing 10 sequential editing evaluation result.")
    ...
    for seq_id in range(NUM_SEQ):
        input_file = f"{args.input_folder}/{seq_id}.json"
    ...
    for gap, acc in enumerate(global_acc_by_gap):
        ...
        for j, loc_audio in enumerate(acc["loc"]["audio"]):
            ...
            print(f"  - Type {j + 1}: {loc_audio_acc:.6f} ({loc_audio['acc']} / {loc_audio['total']})")
